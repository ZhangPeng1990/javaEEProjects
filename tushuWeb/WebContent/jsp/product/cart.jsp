...
<link type="text/css" rel="stylesheet" href="<%=path%>/styles/css/base.css?1379243658">
<link type="text/css" rel="stylesheet" href="<%=path%>/styles/css/common.css?1379243658">
...
<script type="text/javascript" src="<%=path%>/styles/js/js.js?1379243658"></script>
...
<div class="pathBg">
    <div class="w980 m"><a href="/">首页</a>&gt;<a href="/cart.html">购物车</a>&gt;<span>我的购物车</span></div>
</div>
<div class="contentBox pt10 pb30">
    <div class="m w980">
        <div class="buyTitle_1"><span class="hide"><strong>我的购物车</strong>-支付信息-生成订单</span></div>
        <div class="buyBox fc_gray6 pt20 pb30">
        	<!-- 优惠信息
            <div class="pl60 pt8">
            <span class="start_tip">
            	<a href="http://www.tushu.com/zt/zqj.html?from=shoppingcart" target="_blank">中秋温情特典 全场商品满68元包邮</a>
            </span>
            </div>
            -->
			<form action="<%=path%>/user/${sessionUserName.id}/createOrder.html" method="POST" >
            <div class="pt10 pb30">
                <table class="table_1 w860 m tc fc_gray6">
                    ...
                    <tr id="36162" class="goods_num">
                        <td class="w57">
                        	<input class="vam mr5 keys" checked="checked" id="keys_36162" value="${item.id}" name="pay_ids" type="checkbox">
                        </td>
                        <td>
                            <div class="bookTypeBox" name="6020">
                            <img src="http://platform.tushu.com/book/84985?thumb&amp;wrap=70" alt="">
                            </div>
                        </td>
                        <td>
                        	<a href="<%=path%>/product/details/${item.product.productId}.html" target="_blank">
                        	${item.product.name}
                        	</a>
                        </td>

						<td>
							${item.product.sellPrice}元<input id="old_36162" value="15.00" type="hidden">
						</td>
						
						<td class="fc_orange">
							<span class="price_real" id="price_real_36162">${item.amountPayable}</span>元
							<br><span id="td_36162"></span>
							<input id="real_36162" value="9.90" type="hidden">
						</td>
								
                        <td>
                        	<input class="jj_jian" onclick="setCount.reduce(36162)" type="button">
	                        <input id="count36162" value="${item.productNumber}" class="jj_txt" readonly="readonly" type="text">
	                        <input class="jj_add" onclick="setCount.add(36162)" type="button">
                        </td>
                ...
                <div class="clearfix w860 m pt10 tr hide" id="show_pm34" style="display:none;">
                <span class="pr10 fs13 fc_red">享受优惠策略：<span id="pmethod34"></span></span>
                </div>
                <div class="clearfix w860 m pt20">
                    ...
                    <span class="r ff_yahei fs13 lh26 pr30 mr10">商品总额：￥<span class="fs17 fc_orange" id="price_all_cart">${totalAmount}</span>元</span>
                </div>
                <div class="clearfix w780 m tr pt30">
                	<!--
                	<a href="/lists-21000.html" class="btn_gray5 mr15 vam">继续制作</a>
                	
                	<input class="btn_orange vam js_goto_pay" value="去结算" type="button"> 
                	-->
                	<input class="btn_orange vam" value="去结算" type="submit">
                </div>
            </div>
			</form>
...
<div id="dialog_5" style="display:none;">
    <div class="w450 pt60 pb30 tc fs14 lh28">确定从购物车中删除？<br>您的涂书作品已经保存到“我的作品”中，<br>可随时再次添加到购物车。</div>
</div>
...
$(document).ready(function(){
	var share = $("#dialog_8").attr("share");
	if( share == 1){
		adialog('隐私与分享',document.getElementById('dialog_8'));
        var op_save_privacy = getCookie('default_book_privacy');
        if (op_save_privacy) {
            $('#cb_save_privacy').attr('checked', true);
            $('#select_privacy').val(op_save_privacy);
        }
	}
    $('#cb_save_privacy').click(function(){
        var v = $(this).attr('checked');
        var value = v ? 1 : 0;
        var selectPrivacy = $('#select_privacy').val();
        if (value > 0) {
            setCookie('default_book_privacy', selectPrivacy, 7);
        }
        else {
            setCookie('default_book_privacy', '', -7);
        }
    });
	//share
	$('.js_share_button').bind('click', function(e){
        $('#cb_save_privacy').trigger('click');
		var checkbox  = $(".js_share_checkbox").attr('checked');
        var iselect_privacy = $('#select_privacy').val();
        var save_default_privacy = $('#cb_save_privacy').val();
        var shareTypes = [];
        var checkedBoxes = $('.js_share_checkbox:checked').each(function(){
            if ($(this).attr('stype')) {
                shareTypes.push($(this).attr('stype'));
            }
        });
		//var content  = $(".js_share_textarea").val();
        var content  = '' + 'http://www.tushu.com/book-84985.html';// $(".js_share_textarea").val();
        var share_option = $('#select_privacy').val();
        //$.ajax({type:"POST", dataType:"json", url:"/user/update_acl", data:"book_id="+84985+"&acl=" + share_option+"&m="+Math.random(),success:function(data){}});
		if(checkbox == 'checked')
		{
			$.ajax({
				type: "POST",
				dataType:'json',
				url: "/home/post_share",
				data:"is_share=1&share_option="+(share_option==2?1:0)+"&book_type=6020&bookid=84985&pic=http%3A%2F%2Fplatform.tushu.com%2Fbook%2F84985%3Fthumb%26wrap%3D400&content="+encodeURIComponent(content)+"&share_types=" + shareTypes.join(',') + "&pic_path=http%3A%2F%2Fplatform.tushu.com%2Fbook%2F84985%3Fthumb%26wrap%3D400",
				success: function(){}
			});
		}
		else
		{
			$.ajax({
				...
				dataType:'json',
				url: "/home/post_share",
				data:"is_share=0&share_option=" + (share_option==2?1:0) + "&bookid=84985&content="+encodeURIComponent(content),
				success: function(){}
			});
		}
        close_dialog();
