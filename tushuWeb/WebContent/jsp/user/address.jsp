...
<link type="text/css" rel="stylesheet" href="<%=path%>/styles/css/base.css?1379484625">
<link type="text/css" rel="stylesheet" href="<%=path%>/styles/css/common.css?1379484625">
...
<script type="text/javascript" src="<%=path%>/styles/js/js.js?1379484625"></script>
...
<body>
<input id="forwardUrl" value="<%=path%>" type="hidden"/>
<c:choose>
...
<div class="pathBg">
    <div class="w980 m"><a href="###">首页</a>&gt;
    					<a href="###">会员中心</a>&gt;<span>我的地址</span></div>
</div>
<div class="contentBox pt10">
    <div class="m w980">
        <div class="clearfix pb10">
            <div class="l photoBg2" id="test00">
                <a href="###"><span class="photoTxt">修改头像</span>
    ...
    <div class="m w980">
        <div class="clearfix selectedBox2">
            ...
            <div class="r selectRight2">
                <div class="selectListBox bst4 fc_gray4 lh26 pl40">
                    <div class="clearfix pt30">
                        <label class="fs15 l tr w190 mr10"><strong class="fc_orange vam">*</strong> 收货人：</label>
                        <input class="l inpBox w220 h20" id="uname" value="" type="text">
                        <p></p>
                    </div>
                    <div class="clearfix pt20">
                        <label class="fs15 l tr w190 mr10"><strong class="fc_orange vam">*</strong>省份：</label>
                        <div class="l">
                        	<!--  
                            <select id="province_id">
                                
                            </select>
                            <select class="ml10" id="city_id">
                                <option value="" id="city_"></option>
                            </select>
                            <select class="ml10" id="county_id">
                                <option value="" id="county_"></option>
                            </select>
                            -->
                            <div>
                    ...
                    <div class="clearfix pt20">
                        <label class="fs15 l tr w190 mr10"><strong class="fc_orange vam">*</strong> 地址：</label>
                        <input class="l inpBox w350 h20" id="address" value="" type="text">
                        <p></p>
                    </div>
                    <div class="clearfix pt20">
                        <label class="fs15 l tr w190 mr10"><strong class="fc_orange vam">*</strong> 手机号码：</label>
                        <input class="l inpBox w120 h20" id="mobiel" value="" type="text">
                        <label class="fs15 l mr10 ml10">或 固定电话</label>
                        <input class="l inpBox w120 h20" id="tel" value="" type="text">
                        <p></p>
                    </div>
                    <div class="clearfix pt20">
                        <label class="fs15 l tr w190 mr10">电子邮件：</label>
                        <input class="l inpBox w220 h20" id="email" value="" type="text">
                        <p class="l ml10 fc_gray9">用来接受订单提醒邮件，便于您及时了解订单状态</p>
                    </div>
                    <div class="clearfix pt20">
                        <label class="fs15 l tr w190 mr10">邮编：</label>
                        <input class="l inpBox w220 h20" id="code" value="" type="text">
                        <p class="l ml10 fc_gray9">有助于快速确定送货地址</p>
                    </div>
                    <div class="pt20 ml10 pl190 clearfix">
                        <a href="javascript:;" onclick="save_consignee(${sessionUserName.id});" class="l btn_gray3 m0">添加到常用地址</a>
                        <p class="l ml10 fc_gray9">注：标注<strong class="vam ml5 mr5 fc_orange">*</strong>为必填项</p>
                    </div>
                    <div class=" pt30 pb30 pl190 ml10"><input class="btn_orange" onclick="save_consignee(${sessionUserName.id})" id="ts_keep_save" value="保存收货人信息" type="button"></div>
                    <input name="consignee_id" id="consignee_id" value="" type="hidden">
                </div>
                <div class="pl30 pt20 pb20"><strong class="fc_gray6 fs17 ff_yahei ml10">地址列表</strong><span class="fc_gray9 ml10">（最多可保存<span class="fc_orange">5</span>个收货地址）</span></div>
                <div class="pb30">
                    <table class="table_1 w780 m tc fc_gray6" id="address_list">
                        <tbody>
                        <tr>
                            <th>默认地址</th>
                            <th>收货人</th>
                            <th>所在地区</th>
                            <th class="w220">接收地址</th>
                            <th>邮编</th>
                            <th>联系电话</th>
                            <th>操作</th>
                        </tr>
                        <c:forEach var="item" items="${list}">
                       	<tr>
                           <td><input name="show-address" onclick="get_address(14918)" type="radio"></td>
                           <td>${item.recipientsName}</td>
                           <td>${item.province}、${item.city}</td>
                           <td class="tl">${item.county}、${item.street}</td>
                           <td>${item.postCode}</td>
                           <td>${item.phone}</td>
                           <td><a href="<%=path%>/user/deleteAddress/${sessionUserName.id}/${item.id}.html" onclick="return confirm('确认删除吗?');" class="fc_orange">删除</a></td>
                        </tr>
                        </c:forEach>
                       </tbody>
                       </table>
...
<script type="text/javascript" src="<%=path%>/styles/js/consignee.js"></script>
<script type="text/javascript">
    //验证
    var verTrue  = "l txt_full";
    var verFalse = "l txt_err";
    var verTip = "l ml10 fc_gray9";
    var tipemail = $("input#email").next("p").html(),tipcode = $("input#code").next("p").html();

    $("input#uname").bind("focus",verFouus).bind("blur",verUser);
    $("input#address").bind("focus",verFouus).bind("blur",verAddr);
    $("input#mobiel").bind("focus",verFouus).bind("blur",verTel);
    $("input#tel").bind("focus",verFouus).bind("blur",verTel);
    $("input#email").bind("focus",verOtherFouus).bind("blur",verEmail);
    $("input#code").bind("focus",verOtherFouus).bind("blur",verCode);



    function verOtherFouus(){
        var tid = $(this).attr("id");
        $(this).next("p").attr("class",verTip).html(eval("tip"+tid));
    }
    function verFouus(){
        $(this).next("p").removeClass().attr("class",verTip).html("");
    }
    function verUser(){
        var v = $(this).val();
        if(v == ""){
            $(this).next().attr("class",verFalse).html("不能为空");
        }else if(v.length > 18){
            $(this).next().attr("class",verFalse).html("收货人过长");
        }else{
            $(this).next().attr("class",verTrue).html("&nbsp;");
        }
    }
    function verAddr(){
        var v = $(this).val();
        if(v == ""){
            ...
            $(this).next().attr("class",verFalse).html("地址过长");
        }else{
            $(this).next().attr("class",verTrue).html("&nbsp;");
        }
    }
    function verTel(){
        ...
        var v2 = $("#tel").val();
        if(v1 == "" && v2 == ""){
            $("#tel").next().attr("class",verFalse).html("不能为空");
        }else if(v1.length > 11){
            $("#tel").next().attr("class",verFalse).html("手机号码过长");
        }else{
            $("#tel").next("p").attr("class",verTrue).html("&nbsp;");
        }
    }
    function verAddr(){
        var v = $(this).val();
    ...
    function verEmail(){
        ...
        if(v == ""){
            $(this).next().attr("class",verTip).html(eval("tip"+$(this).attr("id")));
        }else if($email.test(v) == false){
            $(this).next().attr("class",verFalse).html("格式错误");
        }else{
            $(this).next().attr("class",verTrue).html("&nbsp;");
        }
    }
